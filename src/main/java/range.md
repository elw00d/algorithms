...
Для правильного декодирования нам нужно поддерживать low и range на каждом шаге в том же виде, что и при кодировании,
а также иметь value, находящееся между low и low+range. Декодирование очередного символа мы можем делать по аналогии с
арифметическим декодером, измеряя интервал между value и low. Процесс осложняется тем, что мы по сути работаем с
31-битными числами, но в рамках 32-битных контейнеров. Поэтому мы должны всегда четко следить за тем, какое значение
у нас принимают старшие биты low и value. Старший бит low может принимать значение 1, но только до нормализации.
Старший бит value получается сдвигом value на 8 бит влево, и тоже может принимать значение 1. Так как для успешного
декодирования очередного символа нам нужно получить корректную разность value - low, то нам нужно следить за тем, чтобы
value всегда был строго между low и low+range. Как это сделать ? Нас интересует то, какие биты мы присвоим value при
очередной нормализации, т.к. если после нормализации условие low <= value < low+range выполняется, то до следующей
нормализации оно не будет нарушено (т.к. мы только сужаем интервал). Рассмотрим следующие случаи нормализации:

1. Рабочий интервал не превышает MIN_RANGE / 2 (это означает, что 23ий бит low и low+range-1 одинаковые),
   23-ий бит low = 0. При нормализации low <<= 8, range <<= 8, переполнения нигде не возникает, поэтому
   старший бит value после нормализации должен быть равен нулю автоматически (т.к. до нормализации
   low <= value <= low+range-1).

2. Рабочий интервал не превышает MIN_RANGE / 2, но 23-ий бит low = 1. При нормализации low <<= 8 мы получим
   число с установленным старшим 32-ым битом, и будем вынуждены его принудительно сбросить, как мы это делали
   при кодировании, для того, чтобы low+range-1 всегда помещался в 32 битах типа int. Но наш value указывал
   на то число, которое было справа от границы полуинтервала, и при сдвиге на 8 бит влево в нём будет установлен
   32-ой бит. Нам нужно его сбросить, чтобы low <= <= low+range-1 продолжало выполняться.

3. Рабочий интервал больше MIN_RANGE / 2 и 23-ий бит low = 0, а 23-ий бит low+range-1 = 1.
   При нормализации low старший бит сбрасываться не будет, поэтому value мы можем не трогать - его
   старший бит должен автоматически корректно показывать на нужное число.

4. Рабочий интервал больше MIN_RANGE / 2 и 23-ий бит low = 1, а 23-ий бит low+range-1 = 0.
   При нормализации low будет отброшен старший бит, таким образом рабочий интервал "смещается"
   влево на MIN_RANGE / 2. Значит, нам нужно соответствующим образом скорректировать value,
   чтобы наш инвариант low <= <= low+range-1 продолжал быть истинным. Так как value в этой ситуации
   на 23-ем бите может иметь как 0, так и 1 (в зависимости от того, как дальше будет уточняться интервал),
   то нам нужно инвертировать этот бит. После этой операции value продолжит указывать на число
   внутри рабочего интервала.

Итак, мы можем записать в нормализации:
...
Это можно оптимизировать. Так как на самом деле всё, что мы делаем - для того, чтобы правильно
посчитать разность (value - low) в 31-битовом формате, и 32-ой бит нам только мешает (но
мы не можем вообще от него отмахнуться, так как нам надо поддерживать состояние рабочего интервала
в том же виде, в котором мы с ним работали при кодировании), то
мы можем воспользоваться более простой формулой, которая работает всегда корректно:

  (value - low) & 0x7fffffff

При этом не нужно следить за старшим битом value, он может быть любым. За старшим битом low
следить всё-таки приходится, но только для того, чтобы число low+range-1 всегда помещалось в int.
Это, как и при кодировании, делается при нормализации.

5) Правила при работе с unsigned числами в java:

Эти моменты подробно описаны в статье http://habrahabr.ru/post/225901/

todo : Проблема, связанная с возможным переполнением переменной carryCount - актуальна
как для арифметических кодеров, так и оригинального интервального кодера. Для carryless
range coder'a, где от переноса отказались, этой проблемы не существует.